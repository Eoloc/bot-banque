...
class MoneyUserGive50Command extends commando.Command {
    ...
    constructor(client) {
        super(client, {
            ...
            group: 'bank',
            memberName: 'donner50',
            description: '  b!donner50 [nom] | Permet de donner un billet de 50 € a quelqu\'un.'
        });
    }

    async run(message, args) {
      bank = accessBank.get();
        if (message.content.startsWith("b!donner50 ")){
            var argument = message.content.substr("b!donner50 ".length);
            var tabNom = [];
            var nomDonneur;
            var nomReceveur;
            var idDonneur;
            ...
            if(tabNom.length == 1){
                ...
                for(var i in bank){
                    if(message.member.id == bank[i].id){
                        bank[i].argent = bank[i].argent - 50;
                ...
                if(nomDonneur == tabNom[0]){
                    ...
                    message.channel.sendMessage("<@" + idDonneur + "> donne 50 € a <@" + idReceveur + ">" );
                }
            }
            if(tabNom.length > 1){
        ...
        console.log(message.member.user.tag + " a fait la commande donner50");
    }
}

module.exports = MoneyUserGive50Command;
