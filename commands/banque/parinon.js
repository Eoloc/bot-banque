...
class PariNonCommand extends commando.Command {
    constructor(client) {
        super(client, {
            name: 'parinon',
            group: 'bank',
            memberName: 'parinon',
            description: '  Permet de miser contre le pari en cours'
        });
    }
    async run(message, args) {
        ...
        if (estDedans) {
            ...
            if(dansBanque){
              console.log(pari);
              pari.push({ id: message.member.id, nom: message.member.user.tag, vote: 1 });
              ...
              message.reply("Vous avez mit " + pari[0].mise + " € sur le tas d'argent et vous avez voté en défaveur du pari en cours.\n"
        ...
        console.log(message.member.user.tag + " a fait la commande parinon");
    }
}
module.exports = PariNonCommand;
